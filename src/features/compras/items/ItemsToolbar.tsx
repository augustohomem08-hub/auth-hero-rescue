import { Search, X, SlidersHorizontal, ArrowRightLeft, Trash2, Copy } from 'lucide-react';
...
interface ItemsToolbarProps {
  ...
  onFiltersChange: (next: ItemsFilters) => void;
...
/** Toolbar above the items list: search, filters, sort, and a bulk-actions bar. */
export function ItemsToolbar({
  ...
  const set = <K extends keyof ItemsFilters>(key: K, value: ItemsFilters[K]) =>
  ...
  const clearAll = () => onFiltersChange({ ...filters, search: '', status: 'all', priority: 'all', category: 'all' });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Buscar por nome, loja, observação…"
        ...
        <Select
          aria-label="Ordenar"
          value={filters.sort}
          onChange={(e) => set('sort', e.target.value as ItemSortKey)}
          className="sm:w-52"
        >
          {ITEM_SORTS.map((s) => (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <FilterSelect value={filters.status} onChange={(v) => set('status', v as ItemsFilters['status'])}>
          <option value="all">Todos status</option>
          {ITEM_STATUSES.map((s) => (
            <option key={s.key} value={s.key}>{s.label}</option>
          ))}
        </FilterSelect>
        <FilterSelect value={filters.priority} onChange={(v) => set('priority', v as ItemsFilters['priority'])}>
      ...
      {selectedCount > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-primary-50 px-3.5 py-2.5 dark:bg-primary-900/30">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="ghost" leftIcon={<Copy className="h-4 w-4" />} onClick={onBulkDuplicate}>
              Duplicar
            </Button>
            <Button size="sm" variant="ghost" leftIcon={<ArrowRightLeft className="h-4 w-4" />} onClick={onBulkMove}>
              Mover
            </Button>
